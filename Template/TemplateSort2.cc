...
template <class T, class Compare = std::less<typename T::value_type>> //萃取
class HeapSort
{
public:
    void sort(T &);
...
private:
    void adjustHeap(T &, int, int);
};

template <class T, class Compare>
void HeapSort<T, Compare>::adjustHeap(T &nums, int parent, int length)
{
    int son = 2 * parent + 1;
    while (son < length)
    {
...
template <class T, class Compare>
void HeapSort<T, Compare>::sort(T &nums)
{
    // build heap O(N)
...
int main()
{
    ...
    print(vstr);
    // HeapSort<string> shs;
    HeapSort<vector<string>, std::greater<string>> shs;
    shs.sort(vstr);
    print(vstr);
